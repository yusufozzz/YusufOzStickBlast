    ...
    public class GridGenerator : MonoBehaviour
    {
        ...
        private void GenerateDots(GridSettingsSo settings, int size, float spacing, float offset, Transform parent)
        {
            for (int y = 0; y < size; y++)
            {
                ...
                {
                    Vector3 pos = GridPos(x, y, spacing, offset);
        ...
        {
            int squareCount = settings.GridSize - 1;
        ...
        private Vector3 GridPos(int x, int y, float spacing, float offset)
        {
            return new Vector3(x * spacing - offset, y * spacing - offset, 0f);
        }
